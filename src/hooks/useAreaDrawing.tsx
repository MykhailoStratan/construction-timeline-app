...
export default function useAreaDrawing(viewer: Viewer | null) {
  ...
  const removeAxisHelper = useCallback(async () => {
    ...
    if (axisAreaRef.current && movedPositionsRef.current) {
      ...
      const result = await computeAreaWithTerrain(
        positions,
        (cartographics) => viewer.scene.sampleHeightMostDetailed(cartographics),
      )
      if (result) {
  ...
  return { startAreaMode, isAreaMode }
}
